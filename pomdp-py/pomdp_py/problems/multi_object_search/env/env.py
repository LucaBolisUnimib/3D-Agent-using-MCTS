...
def interpret(worldstr):
    ...
    objects = set_target()  # objid -> object_state(pose)
    obstacles = set({})  # objid
    ...
    for y, line in enumerate(worldlines):
        ...
        for x, c in enumerate(line):
            ...
            # elif c.isupper():
                ...
                assert c == "." or "T", "Unrecognized character %s in worldstr" % c
    if len(robots) == 0:
...
def set_target():
    objects = {}
    for objid in range(len(maps.dispositions[maps.dispositionid])):
        objects[objid] = ObjectState(objid, "target", (maps.dispositions[maps.dispositionid][objid][1], 
                                                       maps.dispositions[maps.dispositionid][objid][0]))
    return objects