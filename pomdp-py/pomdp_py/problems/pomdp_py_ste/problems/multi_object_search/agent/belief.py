...
def initialize_belief(
    ...
    if representation == "histogram":
        ...
        return _initialize_particles_belief(
            dim, robot_id, object_ids, prior, robot_orientations, num_particles
        )
    else:
...
def _initialize_particles_belief(
    ...
    prior_sum = 0
    for objid in object_ids:
        prior_sum += prior[objid][list(prior[objid].keys())[0]]
    for objid in object_ids:
        particles = [
        ...
        if objid in prior:
            # prior knowledge provided. Just use the prior knowledge
            #print(prior_sum)
            for pose in prior[objid]:#{(5, 4): 1.0}
                #print(pose)
                state = ObjectState(objid, "target", pose)
                #print(type(prior_sum))
                amount_to_add = (prior[objid][pose] / prior_sum) * num_particles
                for _ in range(int(amount_to_add)):
